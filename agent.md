...
6. `frontend_routes.md`  
         - **前端路由的唯一事实来源**  
         - 定义：  
           - 已存在 / 计划存在的路由  
           - 页面所属阶段（F1/F2/F3…）  
           - 页面当前完成度（占位 / 结构完成 / 功能完成）  
         - AI **不得**擅自新增、删除或假设路由存在性
7. `frontend_api_map.md`  
         - **前端可用接口与接入阶段的唯一事实来源**  
         - 定义：  
           - 接口是否存在（Exists）  
           - 已验证方式（Thunder Client / 后端阶段）  
           - 是否已接入前端页面  
         - AI **不得**将“接口存在”误写为“页面已完成业务功能”
...
- AI 不得仅凭代码结构推断项目完成度
- 路由是否存在、页面属于哪个阶段：  
  → **以 `frontend_routes.md` 为唯一事实来源**
- 将“接口存在”表述为“页面功能已完成”
- 将“页面骨架完成”表述为“业务闭环完成”
- 接口是否存在、是否已接入页面：  
  → **以 `frontend_api_map.md` 为唯一事实来源**
